...
def SplitData(df, col, numOfSplit, special_attribute=[]):
    ...
    N = df2.shape[0]    #排除特殊值后剩下的样本数
    n = N//numOfSplit    #n表示多少个切分点
    splitPointIndex = [i*n for i in range(1,numOfSplit)]
    rawValues = sorted(list(df2[col]))
    splitPoint = [rawValues[i] for i in splitPointIndex]
    splitPoint = sorted(list(set(splitPoint)))    #splitPoint表示具体在哪个值上切分，如第100个值正好是0.06，前面的数都是比0.06小的
    return splitPoint
...
def Chi2(df, total_col, bad_col):
    ...
    badChi = [(i[0]-i[1])**2 / i[0] for i in badCombined]    #RuntimeWarning: invalid value encountered in double_scalars
...
def ChiMerge(df, col, target, max_interval=5,special_attribute=[],minBinPcnt=0):
    ...
    if N_distinct <= max_interval:  #如果原始属性的取值个数低于max_interval，不执行这段函数
        ...
        if N_distinct > 100:
            split_x = SplitData(df2, col, 100)  #split_x表示切分点的集合
        ...
        while (len(groupIntervals) > split_intervals):  # 终止条件: 当前分箱数＝预设的分箱数
            ...
            for k in range(len(groupIntervals)-1):
                ...
                df2b = regroup.loc[regroup['temp'].isin(temp_group)]  #相邻两个组计算卡方
                #chisq = Chi2(df2b, 'total', 'bad', overallRate)
                chisq = Chi2(df2b, 'total', 'bad')
                chisqList.append(chisq)  #相邻两组数据的卡方值存储在chisqList
...
def BadRateEncoding(df, col, target):
    ...
    #print('查看br_dict')
    #{'credit_card': {'bad_rate': 0.071820870299957748}, 'debt_consolidation': {'bad_rate': 0.10841998478316003}, 'car': {'bad_rate': 0.081395348837209308}, 'house': {'bad_rate': 0.13375796178343949}, 'moving': {'bad_rate': 0.1357142857142857}, 'renewable_energy': {'bad_rate': 0.1702127659574468}, 'home_improvement': {'bad_rate': 0.097934200459066562}, 'educational': {'bad_rate': 0.13333333333333333}, 'vacation': {'bad_rate': 0.14124293785310735}, 'other': {'bad_rate': 0.13343881856540085}, 'major_purchase': {'bad_rate': 0.084130019120458893}, 'medical': {'bad_rate': 0.14759036144578314}, 'small_business': {'bad_rate': 0.21693121693121692}, 'wedding': {'bad_rate': 0.084905660377358486}}
...
def CalcWOE(df, col, target):
    ...
    WOE_dict = regroup[[col,'WOE']].set_index(col).to_dict(orient='index')   # dict like {index -> {column -> value}}，这里index是col，column是‘woe’
...
def MergeBad0(df,col,target, direction='bad'):
    ...
    for i in range(regroup.shape[0]-1):
        ...
        print('查看col_regroup[i+1]') #['NONE', 'MORTGAGE']
        print(col_regroup[i+1])
        del_index.append(i)
        if direction == 'bad':
    ...
    #print('查看col_regroup2') #[['NONE', 'MORTGAGE'], ['OWN'], ['RENT'], ['OTHER']]
    newGroup = {}
    for i in range(len(col_regroup2)):
        for g2 in col_regroup2[i]:
            newGroup[g2] = 'Bin '+str(i)
    #print('查看newGroup') #{'OTHER': 'Bin 3', 'OWN': 'Bin 1', 'MORTGAGE': 'Bin 0', 'RENT': 'Bin 2', 'NONE': 'Bin 0'}
    return newGroup
