...
class LogisticRegressionRunner:
    def __init__(self):
        ...
        train_data = pd.read_csv(ROOT_DIR + 'featureEngineering/train_WOE_data.csv',header = 0, encoding = 'latin1')[self.featuresInModel + ['y']]

        self.y_train = train_data['y']
        self.X_train = train_data.drop(['y'], axis = 1)
        del train_data
