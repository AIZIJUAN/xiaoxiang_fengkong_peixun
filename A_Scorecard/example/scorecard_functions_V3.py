...
def BinBadRate(df, col, target, grantRateIndicator=0):
    ...
    regroup['bad_rate'] = regroup.apply(lambda x: float(x.bad) * 1.0 / float(x.total), axis=1)
...
def ChiMerge(df, col, target, max_interval=5,special_attribute=[],minBinPcnt=0):
    ...
    if N_distinct <= max_interval:  #如果原始属性的取值个数低于max_interval，不执行这段函数
        ...
        #groupIntervals: [[x1], [x2], .....[xn]]: col下所有去重后的值构成
        split_intervals = max_interval - len(special_attribute)
        while (len(groupIntervals) > split_intervals):  # 终止条件: 当前分箱数＝预设的分箱数
            # 每次循环时, 计算合并相邻组合并后的卡方值。具有最小卡方值的合并方案，是最优方案
            chisqList = []
            for k in range(len(groupIntervals)-1):
                temp_group = groupIntervals[k] + groupIntervals[k+1]
                df2b = regroup.loc[regroup['temp'].isin(temp_group)]
                #chisq = Chi2(df2b, 'total', 'bad', overallRate)
                chisq = Chi2(df2b, 'total', 'bad')  #计算每个相邻两组合并后的卡方值
                chisqList.append(chisq)
            best_comnbined = chisqList.index(min(chisqList))  #找到卡方值最小的index，定位到与该卡方值相关联的两个group
            groupIntervals[best_comnbined] = groupIntervals[best_comnbined] + groupIntervals[best_comnbined+1]  #进行合并
            # after combining two intervals, we need to remove one of them
            groupIntervals.remove(groupIntervals[best_comnbined+1])  #合并后删除原先的group
        groupIntervals = [sorted(i) for i in groupIntervals]  #对于合并后的每个gurop内的值进行排序
...
def MergeBad0(df,col,target, direction='bad'):
    ...
    for i in range(regroup.shape[0]-1):
        col_regroup[i+1] = col_regroup[i] + col_regroup[i+1]   #将badrate最小的两个bin进行合并
        del_index.append(i)
        if direction == 'bad':   #一旦合并后满足条件，则立即停止合并，否则在前次合并基础上再进行新一轮的两个bin的合并
            if regroup['bad_rate'][i+1] > 0:
                break
        else:
            if regroup['bad_rate'][i+1] < 1:
                break
    col_regroup2 = [col_regroup[i] for i in range(len(col_regroup)) if i not in del_index]  #被合并的旧区间 被删除
    newGroup = {}
